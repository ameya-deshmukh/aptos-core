...
// Returns 0 if the latest version is updated.
// Returns 1 if the version is old and it's ok.
// Returns 2 if the version is > expected version.
const CACHE_SCRIPT_UPDATE_LATEST_VERSION: &str = r#"
    local latest_version = redis.call("GET", KEYS[1])
    if latest_version then
        if tonumber(latest_version) == tonumber(ARGV[1]) then
            redis.call("SET", KEYS[1], ARGV[1])
            return 0
        elseif tonumber(latest_version) < tonumber(ARGV[1]) then
            return 2
        else
            return 1
        end
