...
const MAX_DEPTH_FROM_VALIDATORS: u64 = 100; // Nodes that aren't connected to the network
        ...
        RoleType::FullNode => {
            match peers_and_metadata.get_connected_peers_and_metadata() {
                Ok(peers_and_metadata) => {
                    // Go through our peers, find the min, and return a depth relative to the min
                    let mut min_peer_depth_from_validators = MAX_DEPTH_FROM_VALIDATORS;
                    for peer_metadata in peers_and_metadata.values() {
                        if let Some(depth_from_validators) = peer_metadata
                            .get_peer_monitoring_metadata()
                            .depth_from_validators
                        {
                            min_peer_depth_from_validators =
                                min(min_peer_depth_from_validators, depth_from_validators);
                        }
                    }
                    ...
                    min(
                        ...
                        min_peer_depth_from_validators + 1,
                    )
                },
                Err(error) => {
