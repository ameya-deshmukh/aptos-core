...
use crate::{
    ...
    MAX_DEPTH_FROM_VALIDATORS, PEER_MONITORING_SERVER_VERSION,
};
...
async fn test_get_network_information_fullnode() {
    ...
    verify_network_information(&mut mock_client, HashMap::new(), MAX_DEPTH_FROM_VALIDATORS).await;
    ...
    let peer_depth_1 = MAX_DEPTH_FROM_VALIDATORS; // Peer 1 is not connected to anyone else
    let peer_monitoring_metadata_1 = PeerMonitoringMetadata::new(None, None, Some(peer_depth_1));
    peers_and_metadata
    ...
    let peer_depth_1 = 2; // Peer 1 now has other connections
    let peer_monitoring_metadata_1 = PeerMonitoringMetadata::new(None, None, Some(peer_depth_1));
    peers_and_metadata
        .update_peer_monitoring_metadata(peer_network_id_1, peer_monitoring_metadata_1.clone())
        .unwrap();
    ...
    let peer_depth_2 = 0; // The peer is a validator
    let connection_metadata_2 = create_connection_metadata(peer_id_2);
    let peer_monitoring_metadata_2 = PeerMonitoringMetadata::new(None, None, Some(peer_depth_2));
    peers_and_metadata
...
async fn test_get_network_information_validator() {
    ...
    // Process a client request to fetch the network information and verify depth is 0
    ...
    // Update the peer monitoring metadata for peer 1
    let peer_depth_1 = 0; // Peer 1 now has other connections
    let peer_monitoring_metadata_1 = PeerMonitoringMetadata::new(None, None, Some(peer_depth_1));
    peers_and_metadata
        .update_peer_monitoring_metadata(peer_network_id_1, peer_monitoring_metadata_1.clone())
        .unwrap();
    ...
    let peer_depth_2 = 1; // The peer is a VFN
    let connection_metadata_2 = create_connection_metadata(peer_id_2);
    let peer_monitoring_metadata_2 = PeerMonitoringMetadata::new(None, None, Some(peer_depth_2));
    peers_and_metadata
        .insert_connection_metadata(peer_network_id_2, connection_metadata_2.clone())
...
async fn verify_network_information(
    ...
    expected_depth_from_validators: u64,
) {
    ...
    let expected_response =
        PeerMonitoringServiceResponse::NetworkInformation(NetworkInformationResponse {
            ...
            depth_from_validators: expected_depth_from_validators,
        });
    assert_eq!(response, expected_response);
}
